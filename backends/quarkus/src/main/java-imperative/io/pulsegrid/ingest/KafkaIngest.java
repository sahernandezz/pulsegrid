...
/**
 * Queue ingestion (only INGEST_MODE=queue), standard blocking Kafka client, gated at
 * startup by config. The producer returns the id; the consumer runs on a dedicated
 * thread and persists via the blocking {@link EventStore}.
 */
@ApplicationScoped
public class KafkaIngest {
    ...
    public String publish(EventRequest event) {
        ...
        producer.send(new ProducerRecord<>(topic, event.deviceId(), serialize(new QueueMessage(id, event))));
        return id;
    }
    ...
    private Properties consumerProps() {
        ...
        p.put(ConsumerConfig.GROUP_ID_CONFIG, "pulsegrid-quarkus-imperative");
